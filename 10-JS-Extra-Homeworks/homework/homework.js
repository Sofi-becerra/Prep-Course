...
function asAmirror(str) {
  ...
  //Escribe tu código aquí
  var palabraSep = str.split (" ");
  for(var i = 0; i < palabraSep.length; i++){
    palabraSep[i] = palabraSep[i].split("").reverse().join("");
  } return palabraSep.join(" ");
}
...
function deleteAbc(cadena){
  ...
  for(var i = 0; i < cadena.length; i++){
   if( cadena[i] !== "a" && cadena[i] !== "b" && cadena[i] !== "c") {
    nuevaVersion = nuevaVersion + cadena[i];
   } 
  }return nuevaVersion;
}
